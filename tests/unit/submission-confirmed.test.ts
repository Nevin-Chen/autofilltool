...
describe('lever.detectSubmissionConfirmed', () => {
  ...
  it('true on the /thanks route', () => {
    ...
    expect(det(document, new URL('https://jobs.lever.co/stripe/123/thanks'))).toBe(true);
  });

  it('true when the apply form is gone and confirmation copy is present', () => {
    document.body.innerHTML = THANKS;
    expect(det(document, new URL('https://jobs.lever.co/stripe/123'))).toBe(true);
  });

  it('false while the apply form is present', () => {
    document.body.innerHTML = THANKS + '<form action="https://jobs.lever.co/stripe/123/apply"></form>';
    expect(det(document, new URL('https://jobs.lever.co/stripe/123/apply'))).toBe(false);
  });
});

describe('ashby.detectSubmissionConfirmed', () => {
  const det = ashbyAdapter.detectSubmissionConfirmed!;
  const url = new URL('https://jobs.ashbyhq.com/stripe/123');
...
describe('workday.detectSubmissionConfirmed', () => {
  ...
  const url = new URL('https://nvidia.myworkdayjobs.com/careers/job/1/apply');
