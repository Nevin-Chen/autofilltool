...
export async function buildPrompt(
  ...
  // Single flat rules array, organised by section so the next person editing
  // the prompt has somewhere obvious to slot a new line in. Headers are
  // comments only, not sent to the model. The split between FACTUAL
  // GROUNDING (what to do about facts) and VOICE (how to write what you do
  // have) is intentional: both rules are load-bearing and bleed into each
  // other if collapsed.
  const rules = [
    // IDENTITY
